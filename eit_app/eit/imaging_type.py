...
def make_voltage_vector(
    eit_model: EITModelClass, transform_funcs: list, voltages: np.ndarray
) -> np.ndarray:
    """_summary_
...
def make_voltage_from_meas_pattern(
    eit_model: EITModelClass, voltages: np.ndarray
) -> np.ndarray:
    """_summary_
    ...
    Returns:
        np.ndarray: _description_
    """
    meas_voltage = voltages[
        :, : eit_model.n_el
    ]  # get only the voltages of used electrode (0-n_el)
    return eit_model.meas_pattern.dot(
        meas_voltage.T
    ).flatten()  # get the volgate corresponding to the meas_pattern and flatten
...
class Imaging(ABC):
    ...
    def process_data(
        ...
        idx_frame: int = 0,
        extract_voltages: bool = False,
    ):

        self.idx_freqs = [
            list(item.values())[0] for item in self.detail_freqs if item is not None
        ]
    ...
    def pre_process_data(
        self, dataset, eit_model: EITModelClass, idx_frame: int = 0
    ) -> List[np.ndarray]:
        """"""
        # return meas_voltage

    def get_metadata(self, dataset, idx_frame: int = 0):
        """provide all posible metadata for ploting"""

        self.freqs_val = [
            dataset.get_freq_val(idx_freq=_idx_freq) for _idx_freq in self.idx_freqs
        ]
        self.idx_frames = []
    ...
    @abstractmethod
    def get_indexes_for_measurements_extration(self):
        """"""
...
class AbsoluteImaging(Imaging):
    def __init__(
        self, idx_freqs: List[int], ref_frame_idx: int = 0, transform_funcs: list = None
    ) -> None:
    ...
    def get_indexes_for_measurements_extration(self):
        ...
        index_frame= [self.idx_frames[0], self.idx_freqs[0]]
        return[index_frame,index_frame]


class TimeDifferenceImaging(Imaging):
    def __init__(
        self,
        idx_freqs: List[float],
        ref_frame_idx: int = 0,
        transform_funcs: list = None,
    ) -> None:
        if transform_funcs is None:
            transform_funcs = []
...
class FrequenceDifferenceImaging(Imaging):
    ...
    def make_labels(self):
        ...
        t = (
            f" ({self.label_meas[1]}); {self.frame_label(0)} ({self.freq_label(0)} - {self.freq_label(1)})",
        )
