...
class SerialInterface(object):
    ...
    def __init__(self) -> None:
        ...
        self.listen_worker = Poller(
            name="Serial", sleeptime=0.01, pollfunc=self.poll_read
        )
    ...
    def open(
        ...
        try:
            self.serial_port = Serial(
                port_name, str(baudrate), timeout=timeout, write_timeout=write_timeout
            )
    ...
    def poll_read(self):
        ...
        try:
            ...
            ]  # for detction of unplugging and truning off or sofreset
        if self.last_rx_frame:
            self.callback(self.last_rx_frame)

    def get_sciospec_complete_frame(self) -> List[bytes]:
        """[summary]
        ...
        rx_frame = []
        if (
            self.read_nb_of_availables_bytes() >= FRAME_LENGTH_MIN
        ):  # a frame is at least 4 bytes
            rx_frame = self.read_bytes(
                LENGTH_BYTE_INDX + 1
            )  # read up to the length byte
            length_data2read = (
                rx_frame[LENGTH_BYTE_INDX] + 1
            )  # read also the additional "ending CMD Byte"
            while (
                self.read_nb_of_availables_bytes() < length_data2read
            ):  # dangerous ...timer use?
                pass
