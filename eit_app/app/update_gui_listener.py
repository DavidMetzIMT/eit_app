...
from eit_app.app.utils import set_comboBox_items, set_table_widget
...
def handle_device_setup_event(app:Ui_MainWindow, device:SWInterface4SciospecDevice, set_freq_max_enable:bool=True, error:bool=False):
    ...
    app.sBd_frame_rate.setValue(device.setup.get_frame_rate())
    app.sBd_max_frame_rate.setValue(device.setup.get_max_frame_rate())
    app.sB_burst.setValue(device.setup.get_burst())
    app.sBd_exc_amp.setValue(device.setup.get_exc_amp()*1000) # from A -> mA
    app.sBd_freq_min.setValue(device.setup.get_freq_min())
    app.sBd_freq_max.setValue(device.setup.get_freq_max())
    app.sB_freq_steps.setValue(device.setup.get_freq_steps())
    app.cB_scale.setCurrentText(device.setup.get_freq_scale())
...
update_devices_events={ 
    ...
    UpdateDeviceEvents.device_connected:         [handle_device_status_event, True],
    UpdateDeviceEvents.device_disconnected:      [handle_device_status_event, True],
